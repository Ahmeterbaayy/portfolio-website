...
const ProjectCard = ({ project }) => {
  ...
  const githubUrl = project?.github || 'https://github.com/Ahmeterbaayy';
  const viewSiteUrl = project?.viewSite || '#';
  ...
  return (
    <div className="w-full max-w-sm mx-auto space-y-4">
      ...
      <div className="flex flex-wrap gap-1.5">
          {/* React ve Axios'un ortasına Redux kutucuğu ekle */}
          {(() => {
            const stack = project.techStack.map(t => t.toLowerCase());
            const hasReact = stack.includes('react');
            const hasRedux = stack.includes('redux');
            const hasAxios = stack.includes('axios');
            if (hasReact && hasRedux && hasAxios) {
              // React, Redux, Axios sıralamasıyla göster
              return [
                <TechPill key="react" tech={project.techStack.find(t => t.toLowerCase() === 'react')} />,
                <TechPill key="redux" tech={project.techStack.find(t => t.toLowerCase() === 'redux')} />,
                <TechPill key="axios" tech={project.techStack.find(t => t.toLowerCase() === 'axios')} />
              ];
            } else {
              // Redux yoksa veya farklı bir sıralama varsa, mevcut sıralamayı koru
              return project.techStack.map((tech, index) => (
